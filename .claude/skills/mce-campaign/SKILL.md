...
> **수동 모드의 모든 결정 지점은 `AskUserQuestion` 도구로 묻는다.** 사용자가 텍스트로 번호를 입력하는 대신 **버튼을 클릭**해 고르게 한다.
...
| 지점 | 질문(header) | 클릭 옵션 |
|---|---|---|
| STEP 1-5 캠페인 선택 | `캠페인 선택` | 후보 1·2·3… 각 캠페인명 + `추천` (의도에 맞게 자동 선택) |
| STEP 2 실행 모드 | `실행 모드` | `수동` / `자동` |
| STEP 2 수동 진행 방식 | `진행 방식` | `정의서 후 승인` / `바로 저니 생성` |
| STEP 2 Plan 승인 | `Plan 승인` | `승인 (저니 생성)` / `수정할게요` |

규칙:
- **정보가 많은 후보 표는 먼저 텍스트로 출력**하고, 그 **아래에 `AskUserQuestion`으로 선택만** 받는다(옵션 라벨은 짧게: 캠페인명 또는 번호).
- 옵션 `description`에는 각 선택의 의미·결과를 한 줄로 적는다(예: 복잡도, 다음에 일어날 일).
- 사용자가 버튼 대신 직접 타이핑("3번", "추천", "수정")해도 동일하게 처리한다.
...
> 🔘 **클릭형 선택**: 후보 표를 텍스트로 출력한 뒤, `AskUserQuestion`(header `캠페인 선택`)으로 **각 후보(캠페인명) + `추천`** 을 옵션으로 제시한다. 각 옵션 `description`에 복잡도·한 줄 설명을 넣는다. 사용자가 텍스트로 번호/`추천`을 입력해도 동일 처리.
